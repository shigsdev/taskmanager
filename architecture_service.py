...
_ER_TABLE_GROUPS: dict[str, str] = {
    ...
    "import_log": "ops",
    "app_settings": "ops",
    "global_context_files": "ops",  # #336 reference docs on every reflection
    ...
    # `make_google_blueprint`), so OAuth identity lives in the signed
    # Flask session cookie, never a DB row. The fictional entry has
    # been removed from here, `_ER_TABLE_ORDER`, and
...
# The ~200-line plain-English `_SCHEMA_DESCRIPTIONS` data dict was moved
# to its own data module (#198) so this module stays logic-focused. It is
# re-exported here so every existing `_SCHEMA_DESCRIPTIONS[...]` reference
# (and any test importing it from `architecture_service`) keeps working.
from architecture_schemas import _SCHEMA_DESCRIPTIONS  # noqa: E402

# Columns NOT covered by `_SCHEMA_DESCRIPTIONS`: the universal `id`,
# `created_at`, `updated_at` (in `_HIDDEN_ER_COLUMNS`);
# `_DESCRIPTION_OPTIONAL_COLUMNS` lists any others we deliberately don't
# surface (rare — server-managed bookkeeping columns that don't help a
# user understand the table).
...
def build_per_table_schema() -> list[dict[str, Any]]:
    ...
    # ∪ tables we describe. (#188: this union previously also pulled in
    # the fictional `flask_dance_oauth` — now that it's gone from
    # `_SCHEMA_DESCRIPTIONS`, the union is just the real model tables.)
    # Iterate in `_ER_TABLE_ORDER` first so related tables cluster.
