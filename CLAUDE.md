...
| A new database column / enum member | Update `ARCHITECTURE.md` PostgreSQL box in the diagram to list the new column / enum member; update the matching Components bullet. The ER diagram on `/architecture` (#42) is auto-generated from `db.Model.registry` so the schema view updates automatically. If the column is also user-visible (new tier, new field on the detail panel), update `templates/docs.html` (Help) per the User-visible behavior row below. |
| **A new SQLAlchemy `db.Model` subclass** (new table, e.g. a `Note` model) | The ER diagram on `/architecture` (#42) auto-detects the table, BUT the visual color grouping is hand-curated (#43): add the new table name to `_ER_TABLE_GROUPS` in `architecture_service.py` (`"core"` for user-facing entities, `"ops"` for system records, `"auth"` for token storage), AND to `_ER_TABLE_ORDER` so it clusters near related tables. ALSO add a per-table entry to `_SCHEMA_DESCRIPTIONS` (#44) with `blurb` + `columns: {col_name: {desc, notes, fk_target?}}` so the plain-English per-table card renders. Tests `test_every_model_table_has_a_group` + `test_every_column_has_a_description` will fail otherwise (drift gates). |
| **A new column on an existing `db.Model`** (incl. SQLAlchemy 2.0 `mapped_column(` / `Mapped[`, not just legacy `db.Column(`) | If the column should appear on the `/architecture` per-table card (#44 — i.e. it's not in `_HIDDEN_ER_COLUMNS = {id, created_at, updated_at}` or `_DESCRIPTION_OPTIONAL_COLUMNS`), add a description to `_SCHEMA_DESCRIPTIONS[table]['columns'][col_name]` — defined in `architecture_schemas.py` (re-exported via `architecture_service.py`); `grep -rn "_SCHEMA_DESCRIPTIONS ="` to find the real definition site. For FK columns, include `fk_target` so the "→ links to" text renders. The drift-gate test `test_every_column_has_a_description` fails if a column has no entry. |
| **A new gate added to `scripts/run_all_gates.sh`** (or a new drift-prevention check) | Update the "11 quality gates" table OR the "Drift-prevention gates" table on `/architecture` (#54) — the in-app testing docs section. Both tables live in `templates/architecture.html`. If the new gate ties to a specific bug it prevents, document that pairing in the Prevents-recurrence-of column so future-you knows why it's there. Also add to the Engineering details collapsible at the bottom of that section if the run-command syntax changed. |
| **User-visible behavior** (capture-bar shortcut, tier rule, voice memo prompt behavior, recurring spawn timing, error message shown to user, button label, time window the user sees, new feature affordance) | Update `templates/docs.html` (the Help page, #33/#40) — find the section that documents this behavior, update it, walk the user-facing fact-check SOP per CLAUDE.md (every claim cited to `file:line`). If the behavior is genuinely new, add a section. Verify at desktop + mobile in Phase 6. The fact-check SOP exists because user-facing copy that drifts from code is a worse bug than missing copy. |
| **A process flow that has a sequence diagram on `/architecture`** (today: recurring spawn, voice memo, auth — others added later by editing `templates/architecture.html`) | The Mermaid sequence diagram in `templates/architecture.html`. Diagram steps must match the actual code path (re-walk per the fact-check SOP). The hand-written prose under each diagram must also still be accurate — including the "Related ADRs" list. If you added a new flow worthy of a diagram, draft one + add a TOC entry under the "Process flows" group. |
...
| **Extending a feature from one task type to multiple** (e.g. work-only → work + personal, like personal projects extending work-only projects) | grep `type === "work"` AND `type == "work"` (and the personal equivalents) across `static/*.js`, route guards, and templates — find every place the old single-type assumption was hard-coded and decide whether each gate is still valid. Bug #57 (2026-04-25): a stale `type === "work"` in the task-detail save handler silently dropped `project_id` for personal tasks, no error raised, only caught by manual user testing. The fix added a payload round-trip test (`tests/js/unit/task_detail_payload.test.js`) and a Playwright assignment-persistence test — when adding a similar feature in the future, write the round-trip assertion FIRST, then implement. |
