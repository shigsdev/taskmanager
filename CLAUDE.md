...
| **A new column on an existing `db.Model`** | If the column should appear on the `/architecture` per-table card (#44 — i.e. it's not in `_HIDDEN_ER_COLUMNS = {id, created_at, updated_at}` or `_DESCRIPTION_OPTIONAL_COLUMNS`), add a description to `_SCHEMA_DESCRIPTIONS[table]['columns'][col_name]` in `architecture_service.py`. For FK columns, include `fk_target` so the "→ links to" text renders. The drift-gate test `test_every_column_has_a_description` fails if a column has no entry. |
