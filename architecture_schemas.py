...
_SCHEMA_DESCRIPTIONS: dict[str, dict[str, Any]] = {
    ...
    "reflections": {
        ...
        "columns": {
            ...
            "continued_from_id":      {"desc": "The reflection this one was continued from (#334)", "notes": "NULL for an ordinary reflection. Continuing a past sitting FORKS it: its transcript, voice segments and attached documents seed a NEW draft and this column points back at the original, which is never modified — the /reflection page and the Help page both promise every reflection is kept forever. ON DELETE SET NULL, so purging a parent one day costs the lineage rather than raising a ForeignKeyViolation.", "fk_target": "reflections"},
            "synthesis_of":           {"desc": "The reflections this row is a COMBINED analysis of (#335)", "notes": "NULL for an ordinary reflection. A JSON list of reflection ids rather than a foreign key, because unlike continued_from_id (one parent, whose words flow into the row) this records that the user read N sittings together on some day -- no text flows and there is no single parent. The row's own transcript is a short header naming the sources; their full text goes to Claude at prompt time and is not copied here. Synthesis rows are skipped by the continuity block, and #338 re-analyze re-reads the sources rather than the header."},
