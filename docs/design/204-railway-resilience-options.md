# #204 — Railway Resilience: Options Analysis

**Status:** Pre-decision design doc. No code commitment yet. Reviewed → choose
direction → split into sub-PRs against #204.

**Filed:** 2026-05-19 during the Railway / GCP US East outage.

**Relationship to neighboring backlog items** — the cron-replay cluster
(#166 resolved, #167–#169 follow-ups) covers the **server-side recovery
path** after Railway comes back (replay missed midnight jobs, auto-self-
heal on startup, operator papercuts when invoking the replay script).
This row is about the **user-side access path** during the outage itself
— what does the user see, can they read their data, can they capture
new work. Both directions matter; this doc only addresses the user-side
half.
...
- Midnight cron jobs (`tomorrow_roll`, `promote_due_today`,
  `realign_tiers`, `recurring_spawn`) silently skipped — surfaced as the
  parallel #166 / #167 / #168 / #169 cluster of backlog rows for server-
  side replay tooling
...
Total: ~5–6 days of work, spread across three sub-PRs against #204. Each
...
- **Server-side cron-replay after outage** — that's #166 (shipped),
  #167 (auto-self-heal), #168/#169 (operator polish). This doc is
  strictly about user-side access during the outage.
